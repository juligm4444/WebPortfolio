  ...
  return (
    <div className="relative w-full max-w-7xl mx-auto py-12">
      ...
      <div className="text-center mb-24">
        <h2 className="text-4xl md:text-5xl font-bold mb-4 text-primary">{title}</h2>
        <p className="text-lg text-muted-foreground font-light max-w-3xl mx-auto">{description}</p>
      </div>

      {/* 3D Carousel Container */}
      <div
        className={`relative mt-10 sm:mt-12 mb-12 sm:mb-16 ${
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            {images.map((image, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`absolute ${cardClasses} transition-all duration-700 ease-out ${
                    style.opacity > 0 ? 'cursor-pointer' : 'pointer-events-none'
                  }`}
                  style={{
